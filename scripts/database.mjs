...
export const databasePath = () => resolve(process.env.NORTHSTAR_DB_PATH || resolve(root, "data/northstar.sqlite"));
...
export function migrate(db) {
  ...
  const applied = hasLedger ? new Set(db.prepare("SELECT version FROM schema_migrations").all().map(({ version }) => version)) : new Set();
