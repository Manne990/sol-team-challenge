...
const forbidden = [
  [/\b(?:describe|it|test)\.(?:only|skip|todo)\s*\(/, "focused, skipped, or todo test"],
...
if (violations.length) throw new Error(`Test policy violations:\n${violations.join("\n")}`);
console.log(`test policy: ${files.length} test files, no focused or skipped tests`);
