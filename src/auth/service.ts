...
export class AuthService {
  ...
  async signIn(
    ...
    const membership = requestedOrganizationId
      ? memberships.find((candidate) =>
          constantTimeEqual(candidate.organizationId, requestedOrganizationId),
        )
  ...
  async logout(token: string | undefined): Promise<void> {
    ...
    if (session && !session.revokedAt)
      await this.repository.revokeSession(session.id, this.now().toISOString());
  }
  ...
  async changeRole(
    ...
    if (
      target.role === "owner" &&
      role !== "owner" &&
      (await this.repository.countActiveOwners(principal.organizationId)) <= 1
    ) {
      throw new AuthError(
        "conflict",
    ...
    await this.repository.revokeSessionsForMembership(
      target.id,
  ...
  private async scopedMembership(
    principal: Principal,
    membershipId: string,
  ): Promise<Membership> {
