...
async function api(path, options) {
  ...
  if (!response.ok || body?.error)
    throw new Error(body?.error?.message || "Notifications are unavailable.");
  return body;
}
...
export function NotificationsPanel() {
  ...
  async function read(notification) {
    if (!notification.readAt)
      await api(`/api/notifications/${notification.id}/read`, {
        method: "POST",
        body: "{}",
      });
    setOpen(false);
  ...
  return (
    <div className="notification-center" ref={panel}>
      <button
      ...
      >
        ...
        {state.unread > 0 && (
          <span className="notification-dot">
            <span className="sr-only">{state.unread} unread</span>
          </span>
        )}
      </button>
      {open && (
        ...
        >
          <div className="notification-panel__heading">
            <h2>Notifications</h2>
            <button onClick={readAll} disabled={!state.unread}>
              Mark all read
            </button>
          </div>
          <div
                ...
                <li className={item.readAt ? "" : "unread"} key={item.id}>
                  <button onClick={() => read(item)}>
                    ...
                    <small>
                      ...
                      UTC{!item.href ? " · Related record unavailable" : ""}
                    </small>
                  </button>
                </li>
              ))}
