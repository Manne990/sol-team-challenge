...
const columns = [
  {
    key: "company",
    label: "Company",
    render: (row) => <a href={`#company-${row.id}`}>{row.company}</a>,
  },
...
export function App({ role = "owner", user, onSignOut }) {
    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <a
          className="brand"
          href="#dashboard"
          aria-label="Northstar CRM dashboard"
        >
        ...
        >
          ...
          <span className="profile-copy">
            Maya Chen<small>{onSignOut ? "Sign out" : role}</small>
          </span>
        </button>
      </header>
      <aside
      ...
      >
        <nav>
          {visibleNavigation.map(([label, id]) => (
            <a
              href={`#${id}`}
        ...
        <div className="workspace">
          <span>NS</span>
          <div>
            Northstar Demo<small>Team workspace</small>
          </div>
        </div>
      </aside>
      {menuOpen && (
          ...
          <>
            <div className="page-heading">
              <div>
                ...
                <h1>Dashboard</h1>
                <p>Here’s what needs attention across your team.</p>
              </div>
              <Button onClick={() => setDialogOpen(true)}>+ Add company</Button>
            </div>
            <section className="metrics" aria-label="Sales overview">
              {[
                ["Open pipeline", "$428,500", "18 active deals"],
                ["Closing this month", "$126,000", "5 deals"],
                ["Overdue tasks", "7", "3 assigned to you"],
                ["Stale accounts", "12", "No activity in 30 days"],
              ].map(([label, value, detail], index) => (
                <a
                  className={index > 1 ? "metric metric--alert" : "metric"}
                  href={index === 2 ? "#tasks?due=overdue" : "#deals"}
                  key={label}
                >
                  <span>{label}</span>
                  <strong>{value}</strong>
            ...
            <section className="panel">
              <div className="panel__heading">
                <div>
                  <h2>Pipeline requiring attention</h2>
                  <p>Open deals ordered by recent activity</p>
                </div>
                <a href="#deals">View pipeline →</a>
              </div>
              <div className="filters">
                <label>
                  <span className="sr-only">Filter deals</span>
                  <input type="search" placeholder="Filter deals…" />
                </label>
                <label>
                  Stage{" "}
              ...
              <section className="panel">
                <div className="panel__heading">
                  <div>
                    ...
                    <p>Browser and server validation share this language.</p>
                  </div>
                </div>
                <form
                  onSubmit={(event) => {
                ...
                >
                  <Field
                    label="Task title"
                    hint="Be specific about the next action."
                  >
              ...
              <section className="panel">
                <div className="panel__heading">
                  <div>
                    ...
                    <p>Deliberate feedback for every outcome</p>
                  </div>
                </div>
                <OperationalState
                  type="conflict"
                  action={
                    <Button variant="quiet">Review latest version</Button>
                  }
                />
              </section>
            </div>
      ...
      <Dialog
        open={dialogOpen}
        title="Discard unsaved company?"
        description="The company details entered in this form will be permanently lost."
