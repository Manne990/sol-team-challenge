...
describe("DashboardPage", () => {
  it("renders evidence and links every headline to its exact filtered records", async () => {
    ...
    expect(
      screen.getByRole("link", { name: /Overdue tasks/ }).getAttribute("href"),
    ).toBe("/tasks?view=overdue");
    expect(
      screen
        .getByRole("link", { name: /Closing in 30 days/ })
        .getAttribute("href"),
    ).toBe(evidence.closingSoonHref);
    expect(
      screen.getByRole("link", { name: /Stale accounts/ }).getAttribute("href"),
    ).toBe(evidence.staleAccounts.href);
    expect(screen.getByText(/currencies remain separate/i)).toBeTruthy();
  });
  ...
  it("refreshes predictably without accumulating prior metrics", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("link", { name: /Overdue tasks/ }).textContent,
      ).toContain("1"),
    );
