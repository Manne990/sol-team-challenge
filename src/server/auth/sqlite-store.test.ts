...
function fixture() {
  ...
  ).run("org-a", "Northstar", "northstar", now, now);
  db.prepare(
    "INSERT INTO organizations(id,name,slug,created_at,updated_at) VALUES(?,?,?,?,?)",
  ).run("org-b", "Outside", "outside", now, now);
  db.prepare(
    "INSERT INTO users(id,email,password_hash,first_name,last_name,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
  ).run(
    "user-a",
    "owner@example.test",
    "scrypt$seed$86b00bec1f8b87006f733f08d8fdb46d74d6000dac275516fb0cbe59bde09bf262a386c7d8f770ea393e50f32a627f025a4e5d83cc427da1c1d036f559b5c927",
    "A",
    "Owner",
    now,
    now,
...
describe("SQLite authorization boundaries", () => {
  it("does not reveal or mutate a foreign membership", async () => {
    ...
    const before = db
      .prepare(
        "SELECT role,status,version FROM memberships WHERE id='member-b'",
      )
      .get();
    await expect(
      store.changeMemberRole({
  ...
  it("atomically protects the last owner and revokes removed member sessions", async () => {
    ...
    expect(
      db.prepare("SELECT status FROM memberships WHERE id='member-a'").get(),
  ...
  it("rejects expired and revoked sessions", async () => {
    ...
    await expect(
      store.findSession("hash-a", "2026-08-05T20:00:00Z"),
    ).resolves.toMatchObject({ role: "owner", organization: { id: "org-a" } });
    await expect(
      store.findSession("hash-a", "2026-08-07T00:00:00Z"),
    ).resolves.toBeUndefined();
    await store.revokeSession("hash-a", "2026-08-05T21:00:00Z");
    await expect(
      store.findSession("hash-a", "2026-08-05T22:00:00Z"),
    ).resolves.toBeUndefined();
  });
});
