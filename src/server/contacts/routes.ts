...
function parseInput(body: unknown): ContactInput {
  ...
  if (!(["lead", "active", "inactive"] as unknown[]).includes(status))
...
const contactJson = (row: Row) => ({
  ...
  owner: row.owner_membership_id
    ? {
        id: String(row.owner_membership_id),
...
const selectContact = `SELECT c.*,co.name company_name,
  trim(coalesce(u.first_name,'') || ' ' || coalesce(u.last_name,'')) owner_name
...
export function contactsRouter(database: SqliteDatabase, auth: AuthService) {
  const router = Router();
  ...
  const mutable = (user: AuthenticatedUser) =>
    auth.requireRole(user, ["owner", "member"]);
  ...
  const verifyRelations = (user: AuthenticatedUser, input: ContactInput) => {
    if (
      input.companyId &&
      !database
        .prepare("SELECT 1 FROM companies WHERE id=? AND organization_id=?")
        .get(input.companyId, user.organization.id)
    )
      throw new AuthError(
        403,
        "FORBIDDEN",
    ...
    if (
      ...
      !database
        ...
        .get(input.ownerMembershipId, user.organization.id)
    )
  ...
  const audit = (
    ...
    database
      ...
      (id,organization_id,actor_membership_id,action,entity_type,entity_id,correlation_id,summary_json,created_at)
      VALUES(?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        randomUUID(),
    ...
    try {
      ...
      try {
        database
          ...
          (id,organization_id,company_id,first_name,last_name,email,phone,job_title,owner_membership_id,status,tags_json,communication_preference,created_at,updated_at)
          VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          )
          .run(
            id,
      ...
      response.status(201).json({
        contact: contactJson(existing(user.organization.id, id)!),
        warnings: duplicateWarnings(user.organization.id, input.email, id),
      });
    } catch (error) {
      next(error);
    }
  ...
  router.get("/:contactId", async (request, response, next) => {
    try {
      ...
      const related = (sql: string) =>
        database.prepare(sql).all(user.organization.id, id) as Row[];
      const activities = related(
        "SELECT id,type,subject,body,occurred_at occurredAt,creator_label creatorLabel FROM activities WHERE organization_id=? AND contact_id=? ORDER BY occurred_at DESC,id DESC LIMIT 50",
      );
      const deals = related(
      ...
      const history = related(
        "SELECT id,action,summary_json summaryJson,created_at createdAt FROM audit_events WHERE organization_id=? AND entity_type='contact' AND entity_id=? ORDER BY created_at DESC,id DESC",
      ).map((event) => ({
        ...event,
  ...
  router.put("/:contactId", async (request, response, next) => {
    try {
      ...
      const before = existing(user.organization.id, id);
      if (!before)
        return response.status(404).json({
          error: { code: "NOT_FOUND", message: "Contact not found." },
        });
      ...
      try {
        database
          .prepare(
            `UPDATE contacts SET company_id=?,first_name=?,last_name=?,email=?,phone=?,job_title=?,owner_membership_id=?,status=?,tags_json=?,communication_preference=?,updated_at=?,version=version+1 WHERE id=? AND organization_id=?`,
          )
          .run(
            input.companyId,
            input.firstName,
  ...
  const archiveAction =
    ...
    async (
      ...
      try {
        const user = mutable(await authenticate(request));
        const id = String(request.params.contactId);
        if (!existing(user.organization.id, id))
          return response.status(404).json({
            error: { code: "NOT_FOUND", message: "Contact not found." },
          });
        const now = new Date().toISOString();
        ...
        try {
          database
            ...
            .run(restore ? null : now, now, id, user.organization.id);
          audit(
            user,
