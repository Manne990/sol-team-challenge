...
test("forward migration creates the complete domain schema", () => {
  ...
  const migrationCount=readdirSync("migrations").filter(name=>name.endsWith(".sql")).length;
  try {
    ...
    assert.equal(fixture.db.prepare("SELECT count(*) n FROM schema_migrations").get().n,migrationCount);
...
test("a failed forward migration rolls back its schema and version record", () => {
  const fixture=isolated();
  const migrations=mkdtempSync(join(tmpdir(),"northstar-migrations-"));
  try {
    writeFileSync(join(migrations,"002_broken.sql"),"CREATE TABLE should_rollback(id TEXT PRIMARY KEY) STRICT; INSERT INTO missing_table VALUES ('failure');");
    assert.throws(()=>migrate(fixture.db,migrations),/Migration 002_broken.sql failed/);
    assert.equal(fixture.db.prepare("SELECT name FROM sqlite_master WHERE name='should_rollback'").get(),undefined);
    assert.equal(fixture.db.prepare("SELECT 1 FROM schema_migrations WHERE version='002_broken.sql'").get(),undefined);
  } finally { rmSync(migrations,{recursive:true,force:true}); fixture.close(); }
});
